...
def test_all_simulation_endpoints():
    """Test all simulation-related endpoints"""
    print("\n" + "="*80)
    print("TESTING ALL SIMULATION ENDPOINTS")
    print("="*80)
    
    # Login first to get auth token
    if not auth_token:
        if not test_login():
            print("❌ Cannot test simulation endpoints without authentication")
    ...
    # Test 7: Pause Simulation
    print("\nTest 7: Pausing the simulation")
    ...
    print("\nTest 8: Resuming the simulation")
    ...
    # Test 9: Get Time Status
    print("\nTest 9: Getting time status")
    
    time_status_test, time_status_response = run_test(
        "Get Time Status",
    ...
    if not time_status_test:
        print("❌ Failed to get time status")
    else:
        print("✅ Successfully retrieved time status")
    
    # Test 10: Next Period
    print("\nTest 10: Advancing to next period")
    
    next_period_test, next_period_response = run_test(
        "Next Period",
    ...
    if not next_period_test:
        print("❌ Failed to advance to next period")
    else:
        print("✅ Successfully advanced to next period")
    
    # Test 11: Toggle Auto Mode
    print("\nTest 11: Toggling auto mode")
    ...
    toggle_auto_test, toggle_auto_response = run_test(
        "Toggle Auto Mode",
    ...
    if not toggle_auto_test:
        print("❌ Failed to toggle auto mode")
    else:
        print("✅ Successfully toggled auto mode")
    
    # Test 12: Get Auto Status
    print("\nTest 12: Getting auto status")
    
    auto_status_test, auto_status_response = run_test(
        "Get Auto Status",
        "/simulation/auto-status",
        method="GET",
        auth=True
    )
    
    if not auto_status_test:
        print("❌ Failed to get auto status")
    else:
        print("✅ Successfully retrieved auto status")
    
    # Test 13: Initialize Research Station
    print("\nTest 13: Initializing research station")
    ...
    # Test 14: Set Language
    print("\nTest 14: Setting language")
    
    set_language_data = {
        "language": "en"
    }
    
    set_language_test, set_language_response = run_test(
        "Set Language",
        "/simulation/set-language",
        method="POST",
        data=set_language_data,
        auth=True
    )
    
    if not set_language_test:
        print("❌ Failed to set language")
    else:
        print("✅ Successfully set language")
    
    # Test 15: Generate Summary
    print("\nTest 15: Generating summary")
    
    generate_summary_test, generate_summary_response = run_test(
        "Generate Summary",
    ...
    if not generate_summary_test:
        print("❌ Failed to generate summary")
    else:
        print("✅ Successfully generated summary")
    
    # Print summary
    print("\nSIMULATION ENDPOINTS SUMMARY:")
    ...
    if all(critical_tests):
        print("✅ All critical simulation endpoints are working correctly!")
        print("✅ Successfully started simulation")
        print("✅ Successfully retrieved simulation state")
        ...
        return True, "All critical simulation endpoints are working correctly"
    else:
        issues = []
        if not simulation_start_test:
        ...
        print("❌ Simulation endpoints have issues:")
        for issue in issues:
            print(f"  - {issue}")
