...
def test_document_bulk_delete_empty_array():
    """Test the document bulk delete endpoint with an empty array"""
    if not auth_token:
        if not test_login():
    ...
    print(f"\nTesting: Document Bulk Delete with Empty Array (DELETE {url})")
    ...
    try:
        ...
        if response.status_code == 200 and "deleted_count" in response_data and response_data["deleted_count"] == 0:
            print("✅ Document bulk delete with empty array works correctly")
            print("✅ Returned 200 with deleted_count=0")
            return True
        else:
            print("❌ Document bulk delete with empty array failed")
...
def test_document_bulk_delete_nonexistent_ids():
    """Test the document bulk delete endpoint with non-existent IDs"""
    if not auth_token:
        if not test_login():
            print("❌ Cannot test document bulk delete without authentication")
            return False
    ...
    url = f"{API_URL}/documents/bulk"
    print(f"\nTesting: Document Bulk Delete with Non-Existent IDs (DELETE {url})")
    
    headers = {"Authorization": f"Bearer {auth_token}"}
    data = ["non-existent-id-1", "non-existent-id-2"]  # Non-existent IDs
    ...
    try:
        ...
        if response.status_code == 404:
            print("✅ Document bulk delete with non-existent IDs works correctly")
            print("✅ Returned 404 for non-existent IDs")
            return True
        else:
            print("❌ Document bulk delete with non-existent IDs failed")
...
def main():
    """Run all tests"""
    print("\n" + "="*80)
    print("TESTING DOCUMENT BULK DELETE ENDPOINT")
    print("="*80)
    
    # Test document bulk delete with empty array
    empty_array_success = test_document_bulk_delete_empty_array()
    
    # Test document bulk delete with non-existent IDs
    nonexistent_ids_success = test_document_bulk_delete_nonexistent_ids()
    
    # Print summary
    print("\n" + "="*80)
    print("DOCUMENT BULK DELETE ENDPOINT ASSESSMENT")
    print("="*80)
    
    if empty_array_success and nonexistent_ids_success:
        print("✅ Document bulk delete endpoint is working correctly!")
        ...
        print("❌ Document bulk delete endpoint has issues:")
