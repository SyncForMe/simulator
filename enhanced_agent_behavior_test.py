...
""",
    ...
    # Test the documents/for-agents endpoint
    documents_test, documents_response = run_test(
        "Get Documents for Agents",
    ...
    if documents_test and isinstance(documents_response, list) and len(documents_response) > 0:
        ...
        print(f"✅ Retrieved {len(documents_response)} documents for agents")
    ...
    if documents_available and isinstance(documents_response, list):
        document_titles = [doc.get("title") for doc in documents_response if "title" in doc]
    ...
    if documents_test and documents_available:
        ...
        print(f"✅ Successfully retrieved documents for agents via /documents/for-agents")
        ...
        if not documents_test:
            issues.append("Failed to retrieve documents for agents")
        if documents_test and not documents_available:
            issues.append("No documents available for agents")
...
""",
    ...
    # Test 1: Propose update with approval
    ...
    # Test 2: Propose update with rejection
    ...
    if approval_update_test and rejection_update_test:
...
def test_generic_document_templates():
    """Test that the templates work for various document types"""
    print("\n" + "="*80)
    print("TESTING GENERIC DOCUMENT TEMPLATES")
    print("="*80)
    
    # Login first to get auth token
    if not auth_token:
        if not test_login():
            print("❌ Cannot test document templates without authentication")
            return False, "Authentication failed"
    ...
    document_types = ["protocol", "training", "research", "reference", "plan"]
    document_results = {}
    
    for doc_type in document_types:
        print(f"\nTesting {doc_type.upper()} document template")
        
        # Create a test document with this type
        document_data = {
            "document_type": doc_type,
            "title": f"Test {doc_type.title()} Document",
            "conversation_context": f"This is a test conversation for creating a {doc_type} document.",
            "creating_agent_id": created_agents[0].get("id"),
            "authors": [agent.get("name") for agent in created_agents],
            "trigger_phrase": f"Let's create a {doc_type}"
        }
        
        doc_test, doc_response = run_test(
            f"Generate {doc_type.title()} Document",
            "/documents/generate",
            method="POST",
            data=document_data,
            auth=True,
        ...
        doc_valid = False
        if doc_test and doc_response.get("success", False):
            content = doc_response.get("content", "")
            
            # Check for type-specific sections
            if doc_type == "protocol":
                doc_valid = "Purpose" in content and "Procedure" in content
            elif doc_type == "training":
                doc_valid = "Objective" in content and "Learning Outcomes" in content
            elif doc_type == "research":
                doc_valid = "Methodology" in content and "Findings" in content
            elif doc_type == "reference":
                doc_valid = "Quick Reference" in content and "Key Information" in content
            elif doc_type == "plan":
                doc_valid = "Objectives" in content and "Implementation Timeline" in content
            
            document_results[doc_type] = doc_valid
            
            if doc_valid:
                print(f"✅ {doc_type.title()} document generated with correct structure")
            else:
                print(f"❌ {doc_type.title()} document missing expected sections")
        else:
            document_results[doc_type] = False
            print(f"❌ Failed to generate {doc_type.title()} document")
    
    # Print summary
    print("\nGENERIC DOCUMENT TEMPLATES SUMMARY:")
    all_templates_passed = all(document_results.values())
    
    if all_templates_passed:
        print("✅ Generic document templates are working correctly!")
        for doc_type in document_types:
            print(f"✅ {doc_type.title()} template: Generated with correct structure")
        return True, "Generic document templates are working correctly"
    else:
        print("❌ Some document templates have issues:")
        for doc_type, result in document_results.items():
            status = "✅" if result else "❌"
            print(f"{status} {doc_type.title()} template")
        return False, "Some document templates have issues"
...
def test_api_endpoints():
    ...
    # Test 1: GET /api/documents/for-agents
    documents_for_agents_test, documents_for_agents_response = run_test(
        "GET /api/documents/for-agents",
        "/documents/for-agents",
        method="GET",
        auth=True
    )
    ...
    # Test 4: POST /api/documents/generate
    # (Already tested in test_generic_document_templates)
    
    # Print summary
    print("\nAPI ENDPOINTS SUMMARY:")
    
    if documents_for_agents_test:
        print("✅ API endpoints are working correctly!")
        print("✅ GET /api/documents/for-agents returns document list for agents")
        ...
        print("✅ POST /api/documents/generate creates documents with proper templates")
        return True, "API endpoints are working correctly"
    else:
        issues = []
        if not documents_for_agents_test:
            issues.append("GET /api/documents/for-agents endpoint failed")
...
def run_all_tests():
    ...
    generic_templates_success, _ = test_generic_document_templates()
    ...
    if all_tests_passed:
        ...
        print("✅ Generic Document Templates: Templates work for various document types")
        ...
        if not generic_templates_success:
            print("  - Generic Document Templates: Issues with document templates")
