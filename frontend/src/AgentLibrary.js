...
const AgentLibrary = ({ isOpen, onClose, onAddAgent }) => {
  ...
  const generateAvatar = async (agent) => {
    try {
      setGeneratingAvatars(prev => ({ ...prev, [agent.id]: true }));
      
      const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/avatars/generate`, {
        method: 'POST',
        headers: {
      ...
      if (result.success && result.image_url) {
        ...
        agent.avatar_url = result.image_url;
      }
    } catch (error) {
      console.error('Error generating avatar:', error);
    } finally {
      setGeneratingAvatars(prev => ({ ...prev, [agent.id]: false }));
    }
  };
                  ...
                  {selectedCategory.agents.map(agent => (
                    <div key={agent.id} className="bg-white border border-gray-200 rounded-lg p-6 hover:shadow-lg transition-shadow">
                      ...
                      <div className="mt-6 space-y-3">
                        {!agent.avatar_url && (
                          <button
                            onClick={() => generateAvatar(agent)}
                            disabled={generatingAvatars[agent.id]}
                            className="w-full px-4 py-2 text-sm border border-purple-200 text-purple-600 rounded-lg hover:bg-purple-50 transition-colors disabled:opacity-50"
                          >
                            {generatingAvatars[agent.id] ? 'Generating Avatar...' : 'Generate Avatar'}
                          </button>
                        )}
